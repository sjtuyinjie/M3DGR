...
namespace zjloc
{
    ...
    void CloudConvert::VelodyneHandler(const sensor_msgs::PointCloud2::ConstPtr &msg)
    {
        ...
        static double tm_scale = 1; //   1e6 - nclt kaist or 1  1e3
        ...
        //zjj587
        bool isSorted = std::is_sorted(pl_orig.points.begin(), pl_orig.points.end(),
                               [](const velodyne_ros::Point &a, const velodyne_ros::Point &b) {
                                   return a.time < b.time;
                               });
        if (!isSorted) {
            std::sort(pl_orig.points.begin(), pl_orig.points.end(), time_list_velodyne);
        }

        // if (pl_orig.points.front().time / tm_scale >= 0.1) {
        //     std::cerr << "All points have time >= 0.1, skipping this frame." << std::endl;
        //     return;
        // }

        pl_orig.points.erase(
            std::remove_if(std::execution::par_unseq, pl_orig.points.begin(), pl_orig.points.end(),
                           [&](const velodyne_ros::Point &point) {
                               return point.time / tm_scale >= 0.1;
                           }),
            pl_orig.points.end());
        plsize = pl_orig.points.size();
        ...
        // pl_orig.points.erase(
        //     std::remove_if(pl_orig.points.begin(), pl_orig.points.end(),
        //                    [&](const velodyne_ros::Point &point) {
        //                        if (point.time / tm_scale > 0.2) {
        //                     //    if ((point.time / tm_scale) - 0.1 > epsilon) { // 使用容差比较
        //                            // 打印被移除点的信息
        //                            std::cerr << "Removing point: time=" << point.time
        //                                      << ", x=" << point.x
        //                                      << ", y=" << point.y
        //                                      << ", z=" << point.z << std::endl;
        //                            return true; // 标记为需要移除
        ...
        // {
        //     std::cerr << "this points have time >= 0.1, skipping this points." << std::endl;
        //     plsize--;
        //     pl_orig.points.pop_back();
        // }
