...
#define USE_ANALYTICAL_DERIVATE 1 //    是否使用解析求导
     ...
     void lidarodom::ProcessMeasurements(MeasureGroup &meas)
     {
          ...
          // std::cout << "Lidar points: " << meas.lidar_.size() << std::endl;
          // std::cout << "IMU data: " << meas.imu_.size() << std::endl;
          ...
          // std::cout << "Frame built with " << const_surf.size() << " points." << std::endl;
     ...
     void lidarodom::poseEstimation(cloudFrame *p_frame)
     {
          ...
          // std::cout << "Pose estimation for frame: " << index_frame << std::endl;
          if (index_frame > 1)
          {
          ...
          // std::cout << "Segmenting FOV..." << std::endl;

          zjloc::common::Timer::Evaluate([&]()
                                         { lasermap_fov_segment(); },
                                         "fov segment");
          // std::cout << "FOV segmentation completed." << std::endl;
     }
                                       ...
                                       std::vector<point3D> &keypoints, const cloudFrame *p_frame)
     {
          ...
          int num_residuals = 0;
          // std::cout << "Number of keypoints(num): " << num << std::endl;
          ...
          for (int k = 0; k < num; k++)
          {
               ...
               std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>> vector_neighbors; // 将变量定义移到 try 块外
               try {
               ...
               vector_neighbors = searchNeighbors(voxel_map, keypoint.point,
                         ...
                         std::cerr << "Exception caught at keypoint " << k << ": " << e.what() << std::endl;
                         break;
               }

               if (vector_neighbors.size() < options_.min_number_neighbors)
               {
                    // std::cout << "Keypoint " << k << " has too few neighbors: " << vector_neighbors.size() << std::endl;
                    continue;
               }
               ...
               // std::cout << "num_closest_neighbors: " << options_.num_closest_neighbors << std::endl;
          ...
          // std::cout << "Total residuals added: " << num_residuals << std::endl;
          // if (num_residuals == 0)
          // return; 
     }
                                ...
                                std::vector<voxel> *voxels)
     {
          ...
          // std::cout << "Searching neighbors for point: " << point.transpose() << std::endl;
          // std::cout << "Voxel map size: " << map.size() << std::endl;
          ...
          for (short kxx = kx - nb_voxels_visited; kxx < kx + nb_voxels_visited + 1; ++kxx)
          {
               for (short kyy = ky - nb_voxels_visited; kyy < ky + nb_voxels_visited + 1; ++kyy)
               {
                    ...
                    {
                         // std::cout << "Current iteration: " << iteration_count << std::endl;
                         ...
                         if (search != map.end())
                         {
                              ...
                              // std::cout << "Voxel (" << kxx << ", " << kyy << ", " << kzz
                              // << ") has " << voxel_block.NumPoints() << " points." << std::endl;
