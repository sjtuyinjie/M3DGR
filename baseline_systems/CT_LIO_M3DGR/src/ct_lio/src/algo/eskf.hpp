...
namespace zjloc
{
    ...
    private:
        ...
        void UpdateAndReset()
        {
            ...
            Eigen::Vector3d omega1 = dx_.template block<3, 1>(6, 0);
            if (!omega1.allFinite()) {
                 std::cerr << "Invalid omega in UpdateAndReset: " << omega1.transpose() << std::endl;
               return;
            }
    ...
    bool ESKF<S>::Predict(const IMU &imu)
    {
        ...
        Eigen::Vector3d omega2 = (imu.gyro_ - bg_) * dt;
        if (!omega2.allFinite()) {
             std::cerr << "Invalid omega in Predict: " << omega2.transpose() << std::endl;
             return false;
        }
        ...
        Eigen::Vector3d omega3 = -(imu.gyro_ - bg_) * dt;
        if (!omega3.allFinite()) {
            std::cerr << "Invalid omega in error state propagation: " << omega3.transpose() << std::endl;
            return false;
        }                       
        // v 对 g
