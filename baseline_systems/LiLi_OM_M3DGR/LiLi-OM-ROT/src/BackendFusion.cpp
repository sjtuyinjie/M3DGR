    ...
    void publishOdometry() {
        if(pose_info_cloud_frame->points.size() >= slide_window_width) {
            ...
            transform.setOrigin(tf::Vector3(
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].x,
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].y,
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].z));
            tf::Quaternion q(
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].qx,
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].qy,
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].qz,
                pose_info_cloud_frame->points[pose_info_cloud_frame->points.size() - slide_window_width].qw);
            transform.setRotation(q);
            br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "odom", "lidar"));
        }
