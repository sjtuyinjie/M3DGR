...
#include "./tool/tools_logger.hpp"
#include "./tool/os_compatible.hpp"

////////////////////////////////////////////////////////
/*** 补充头文件 ***/
#include <glog/logging.h>
#include <sensor_msgs/NavSatFix.h>
#include "GPS/GPSType.hpp"
#include "GPS/MathGPS.hpp"
...
//    deque< double >                     m_img_time_buffer;
    ...
    /*** 补充数据内容 ***/
    cv::Mat m_img;
    string  m_image_topic;
    deque< cv::Mat > m_img_buffer;
    deque< double > m_img_time_buffer;
    void image_cbk(const sensor_msgs::CompressedImageConstPtr &msg);




    GPSData m_gps;  // 当前帧对应的GPS数据
    string m_gps_topic;
    deque<GPSData> m_gps_buffer;
    deque< double > m_gps_time_buffer;
    nav_msgs::Path m_path_msg_gps;
    Eigen::Vector3d m_gps_datum = Eigen::Vector3d::Ones() * NAN;      // GPS数据对应的初始位置(作为后续计算ENU系计算xyz轨迹值的起点)
...
//        m_camera_extrin_R = std::vector< double >( 9, 0.0 );
    ...
    /***  新增函数 ***/
    int service_Camera_update();
    int service_GPS_update();
    void publish_path_gps(const ros::Publisher &pub, Eigen::Vector3d& cur_gps);
