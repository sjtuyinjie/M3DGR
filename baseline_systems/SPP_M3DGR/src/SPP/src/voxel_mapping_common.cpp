///@breif 该文件对应的是voxel_mapping.hpp文件中对应类函数的实现方式
...
void Voxel_mapping::gps_cbk(const sensor_msgs::NavSatFix::ConstPtr &msg)
{
    m_gps.time = msg->header.stamp.toSec();
    /// @attention 注意这里Eigen中输入的数据为纬经高, 并指定噪声数据(根据mins中指定的gps数据值进行确定)
    m_gps.meas << msg->latitude, msg->longitude, msg->altitude;
    m_gps.noise << 0.2, 0.2, 0.4;

    m_mutex_buffer.lock();
    m_gps_buffer.push_back(m_gps);
    m_gps_time_buffer.push_back(msg->header.stamp.toSec());
...
bool Voxel_mapping::sync_packages( LidarMeasureGroup &meas )
{
    ...
    /* imu + lidar + camera 传感器下的数据集打包策略(适用于ImMesh colorMesh的重建工作) */
...
void Voxel_mapping::read_ros_parameters( ros::NodeHandle &nh )
{
    ...
    nh.param< int >( "max_iteration", NUM_MAX_ITERATIONS, 1 );
...
//    nh.param< int >( "img_used", m_img_en, 0 );
    nh.param< string >( "common/lid_topic", m_lid_topic, "/livox/avia/lidar" );
    nh.param< string >( "common/imu_topic", m_imu_topic, "/livox/avia/imu" );
...
//    camera_ext_t_data = {0.0, 0.0, 0.0};
    ...
    // m_extrin_T = {-0.011, -0.02329, 0.04412 };
...
//    cam_k = g_cam_k;
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////
    /*** 新补充内容(camera(全景相机) | GPS | wheel轮计) ***/
    nh.param< string >( "common/image_topic", m_image_topic, "/cv_camera/image_raw/compressed" );
    LOG(INFO) << "Subscribing to topic: " << m_image_topic.c_str();

    // 直接读取ublox_driver中的GPS数据进行分析
    nh.param< string >( "common/gps_topic", m_gps_topic, "/mavros/global_position/raw/fix" );
    LOG(INFO) << "Subscribing to topic: " << m_gps_topic.c_str();
...
void open_log_file()
{
    ...
    {
        string temp = ROOT_DIR;
        Common_tools::create_dir( temp.append( "/ImMesh_output" ));

        // 直接指定当前log文件的输出位置
        std::string cost_time_log_name = temp.append( "/ImMesh_output/mesh_cost_time.log" );
        std::string lio_state_log_name = temp.append( "/ImMesh_output/lio_state.log" );
...
// 补充map_incremental_grow()函数的定义 - 在ImMesh中其是直接被保存在
