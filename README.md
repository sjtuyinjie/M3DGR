...
### 5.6 Download
We offer two download options for each M3DGR sequence: OneDrive (recommended) and Alipan(阿里云盘). If you use Alipan to download sequences as a folder, you need to double-click the ```双击合并.bat``` file in the folder in Win10/Win11 to  merge the original data automatically. You can also download all our public sequences at once through this [link](https://www.alipan.com/s/1EQfYBY5nHM).
